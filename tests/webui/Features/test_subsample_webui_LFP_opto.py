...
# Expected average 
expected_avg = 15.00100261095558
...
def run(playwright: Playwright) -> None:
    ...
    navigate_and_select(page, "/DATOS/pablomc/empirical_datasets/development_EI_decorrelation/opto/LFP")
    ...
    page.get_by_label("Samples axis Dim 0 Dim 1 Dim").first.select_option("2")
    ...
    page.get_by_label("Subject ID source None Custom").select_option("__file_extracted_sep__hyphen__0")
    ...
    page.get_by_role("spinbutton", name="Epoch length (s)").click()
    page.get_by_role("spinbutton", name="Epoch length (s)").fill("3.0")
    page.get_by_role("spinbutton", name="Epoch step (s)").click()
    page.get_by_role("spinbutton", name="Epoch step (s)").fill("3.0")
    ...
    # Obtained average is 13.995853488147578. Maybe because webui doesn't take into account all trials (Opto LFP data has an inhomogenous structure)
...
def test_features_subsample_LFP_opto():
