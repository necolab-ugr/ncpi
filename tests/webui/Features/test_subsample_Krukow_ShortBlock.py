...
### ------------- FEATURE COMPUTATION TEST WITH 10% SUBSAMPLING of Krukow/ShortBlock DATA --------------------
# Inherit common code for all feature tests
from common_features_test_utils import *

class Test_features_subsample_Krukow_ShortBlock(BaseFeatureTest):
    # Keep Path objects for test_tools (it expects Path-like API)
    DATA_DIR = Path('/DATOS/pablomc/empirical_datasets/Krukow_data/Task-Related signals/ShortBlock_Band_1-70/Band_1-70')

    def load_subsample_Krukow_ShortBlock(self, data_dir):
    ...
    loader = load_subsample_Krukow_ShortBlock

    def test_features_subsample_Krukow_ShortBlock(self):
        assert self.compute_mean("catch22") == pytest.approx(5.119589634869177, abs=tolerance)
        assert self.compute_mean("power_spectrum_parameterization") == pytest.approx(
            1.896560219250559, abs=tolerance
        )