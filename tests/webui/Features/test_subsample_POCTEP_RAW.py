...
### ------------- FEATURE COMPUTATION TEST WITH 10% SUBSAMPLING of POCTEP/RAW DATA --------------------
# Inherit common code for all feature tests
from common_features_test_utils import *

class Test_features_subsample_POCTEP_RAW(BaseFeatureTest):
    # Keep Path objects for test_tools (it expects Path-like API)
    DATA_DIR = Path('/DATOS/pablomc/empirical_datasets/POCTEP_data/RAW')

    def load_subsample_POCTEP_RAW(self, data_dir):
    ...
    loader = load_subsample_POCTEP_RAW

    def test_features_subsample_POCTEP_RAW(self):
        assert self.compute_mean("catch22") == pytest.approx(25.429931724459053, abs=tolerance)
        assert self.compute_mean("power_spectrum_parameterization") == pytest.approx(
            2.368143101940835, abs=tolerance
        )