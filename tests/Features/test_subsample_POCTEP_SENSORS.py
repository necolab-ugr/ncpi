...
### ------------- FEATURE COMPUTATION TEST WITH 10% SUBSAMPLING of POCTEP/CLEAN/SENSORS DATA --------------------
# Inherit common code for all feature tests
from common_features_test_utils import *

class Test_features_subsample_POCTEP_SENSORS(BaseFeatureTest):
    # Keep Path objects for test_tools (it expects Path-like API)
    DATA_DIR = Path('/DATOS/pablomc/empirical_datasets/POCTEP_data/CLEAN/SENSORS')

    def load_subsample_POCTEP_SENSORS(self, data_dir):
    ...
    loader = load_subsample_POCTEP_SENSORS

    def test_features_subsample_POCTEP_SENSORS(self):
        assert self.compute_mean("catch22") == pytest.approx(15.690506956008274, abs=tolerance)
        assert self.compute_mean("power_spectrum_parameterization") == pytest.approx(
            2.3322598427365357, abs=tolerance
        )