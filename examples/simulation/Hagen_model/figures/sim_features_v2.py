"""Plot simulation features using data from the second Hagen dataset version."""
...
# Fixed path to the merged Hagen v2 simulation dataset.
sim_data_dir = "/DATOS/pablomc/simulation_datasets/Hagen_v2/Hagen_model_simulations/merged"
...
for method in ['catch22', 'specparam']:
    print(f'\n\n--- Method: {method}')
    ...
    try:
        with open(os.path.join(sim_data_dir, 'data', method, 'sim_theta'), 'rb') as file:
            theta = pickle.load(file)
        with open(os.path.join(sim_data_dir, 'data', method, 'sim_X'), 'rb') as file:
            X = pickle.load(file)
